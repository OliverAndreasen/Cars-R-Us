...
public class MemberService {
  ...
  public MemberResponse addMember(MemberRequest memberRequest) {
    ...
    if (memberRepository.existsById(memberRequest.getUsername())) {
      throw new ResponseStatusException(
        ...
        "Member with this ID already exist"
      );
    }
    ...
    Member member = memberRepository
      ...
      .orElseThrow(() ->
        new ResponseStatusException(
          HttpStatus.BAD_REQUEST,
          "Member with this username already exist"
        )
      );
  ...
  public List<MemberResponse> getMembers() {
    ...
    List<MemberResponse> response = members
      .stream()
      .map(member -> new MemberResponse(member, false))
      .collect(Collectors.toList());

    return response;
  }

  public MemberResponse findMemberByUsername(@PathVariable String username)
    throws Exception {
