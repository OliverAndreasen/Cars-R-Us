...
public class CustomOAuth2AccessDeniedHandler implements AccessDeniedHandler {
  ...
  public static final Logger logger = LoggerFactory.getLogger(
    CustomOAuth2AccessDeniedHandler.class
  );
  ...
  public void handle(
    ...
    if (
      request.getUserPrincipal() instanceof AbstractOAuth2TokenAuthenticationToken
    ) {
      errorMessage =
        "The request requires privileges other than those provided by the access token.";
    ...
    //Refactor
    String wwwAuthenticate = CustomOAuth2AuthenticationEntryPoint.computeWWWAuthenticateHeaderValue(
      parameters
    );
